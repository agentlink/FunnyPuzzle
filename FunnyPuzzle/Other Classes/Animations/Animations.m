...
@property (copy, nonatomic) void (^start)(void);
@end
...
+ (void)scaleIn:(UIView *)view duration:(double)duration start:(void(^)(void))start completion:(void(^)(void))completion
{
    CAAnimationGroup *animationGroup = [CAAnimationGroup animation];
    animationGroup.duration = duration;
...
+ (void)bounceIn:(UIView *)view duration:(double)duration start:(void(^)(void))start completion:(void(^)(void))completion
{
    CAAnimationGroup *animationGroup = [CAAnimationGroup animation];
    animationGroup.duration = duration;
...
+ (void)scaleIn:(UIView *)view duration:(double)duration delay:(double)delay start:(void (^)(void))start completion:(void (^)(void))completion
{
    ...
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
        //code to be executed on the main queue after delay
        if (self) {
            [Animations scaleIn:view duration:duration start:start completion:completion];
        }
    });
}
